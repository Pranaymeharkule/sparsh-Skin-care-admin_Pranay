...
export default function Sidebar({ isOpen, onClose }) {
  ...
  return (
    <>
      ...
      <div
        className={`fixed top-0 left-0 h-full w-72 bg-[#FDE6DD] text-gray-800 flex flex-col shadow-lg z-50  ${
...
function SidebarItem({ icon, label, active = false, onClick }) {
  return (
    <li
      onClick={onClick}
      className={`flex items-stretch gap-3 px-3 py-2 rounded-lg cursor-pointer transition-colors ${
        active ? " text-black" : " text-[#7F7D7D]"
      }`}
    >
      {active && (
        <div className="bg-[#DCC5C6] w-2 rounded-tr-xl rounded-br-xl" />
      )}
